...
export const CesProtocol: React.FC = () => {
    ...
    const resetTimer = useCallback(() => { setTimerRunning(false); setSeconds(0); }, []);
    ...
    useEffect(() => {
        if (jointSideList.length > 0 && !activeJointSide) setActiveJointSide(jointSideList[0].id);
    }, [jointSideList, activeJointSide]);

    // [PRD 4-2] Early return
    if (!session) { navigate('/'); return null; }
    if (jointSideList.length === 0) return <div className="container">Loading...</div>;

    const currentJS = jointSideList.find(js => js.id === activeJointSide) ?? jointSideList[0];
    ...
    const targetMuscles = useMemo(() => {
        ...
        const keywords = ['소흉근', '대흉근', '전방삼각근', '광배근', '상부승모근', '견갑거근', '극하근', '견갑하근', '하부승모근', '전경골근', '비복근', '가자미근', '후경골근', '비골근', '대둔근', '중둔근', '복횡근', '코어', '전거근', 'Y자', 'T자', '케이블', '흉추', '삼각근'];
    ...
    return (
        <div className="ces-dashboard page-bg-ces">
            ...
            <div className="ces-sidebar">
                ...
                <div className="human-anatomy-box" style={{
                    padding: 0,
                    height: '50vh',
                    width: '100%',
                    flex: '0 0 50vh',
                    ...
                    <BodyAnatomySvg
                        ...
                        cesPhase={(activeStage.charAt(0).toUpperCase() + activeStage.slice(1)) as CesPhase}
                    />
                </div>
                ...
                <div className="sidebar-stats">
                    <div className="card stat-card-inner">
                        ...
                        <div className="timer-actions">
                            <button onClick={toggleTimer} className={`btn-timer ${timerRunning ? 'is-running' : 'primary'}`}>
                                {timerRunning ? '⏸ PAUSE' : '▶ START'}
                            </button>
                            <button onClick={resetTimer} className="btn-timer">↺</button>
                        </div>
                    </div>
                ...
                <div className="ces-stage-tabs">
                    {STAGES.map(s => (
                        <button key={s.id} className={`ces-tab-btn ${activeStage === s.id ? 'is-active' : ''}`}
                            onClick={() => { setActiveStage(s.id); setActiveIndex(0); }}>
                            {s.name}
                        </button>
                    ))}
                </div>
                ...
                <div className="sidebar-actions">
                    <button className="btn-complete" onClick={() => navigate('/')}>Complete Exercise <span>›</span></button>
                    <button className="btn-close-circle" onClick={() => navigate('/results')}>✕</button>
                </div>
            </div>
            ...
            <div className="ces-main">
                <div className="flex justify-between items-center mb-8">
                    <select className="form-select" style={{ width: 'auto', boxShadow: 'none', fontWeight: 800, fontSize: '1.25rem' }}
                        value={activeJointSide} onChange={e => { setActiveJointSide(e.target.value); setActiveIndex(0); }}>
                        {jointSideList.map(js => <option key={js.id} value={js.id}>{js.label}</option>)}
                    </select>
                    <div className="main-user-info">
                        ...
                        <div className="user-avatar">
                            <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Richard" alt="avatar" style={{ width: '100%' }} />
                        </div>
                    </div>
                </div>

                <CesExercisePlayer exercises={exercises} stageId={activeStage} activeIndex={activeIndex} onIndexChange={setActiveIndex} />

                {/* 근육 밸런스 */}
                <div className="muscle-balance-box">
                    <h3 className="main-title" style={{ fontSize: '1.1rem', marginBottom: '1.5rem' }}>Muscle Balance Status</h3>
                    <div className="balance-grid">
                        <div className="balance-card">
                            <p className="balance-title" style={{ color: 'var(--danger)' }}>🔴 Overactive (뭉친 근육)</p>
                            <div className="flex flex-wrap gap-2">
                                {analysis.overactiveMuscles.map(m => (
                                    <span key={m} style={{ fontSize: '0.8rem', padding: '0.3rem 0.6rem', background: 'rgba(240,62,62,0.1)', color: 'var(--danger)', borderRadius: '6px', fontWeight: 700 }}>{m}</span>
                                ))}
                            </div>
                        </div>
                        <div className="balance-card">
                            <p className="balance-title" style={{ color: 'var(--success)' }}>🟢 Underactive (약한 근육)</p>
                            <div className="flex flex-wrap gap-2">
                                {analysis.underactiveMuscles.map(m => (
                                    <span key={m} style={{ fontSize: '0.8rem', padding: '0.3rem 0.6rem', background: 'rgba(46,204,136,0.1)', color: 'var(--success)', borderRadius: '6px', fontWeight: 700 }}>{m}</span>
                                ))}
                            </div>
                        </div>
                    </div>
