...
import {
    JOINTS, loadRomSession, saveRomSession,
    getMeasurementQueue, getNextMeasurement,
} from '../lib/romData';
...
export const RomMeasurement: React.FC = () => {
    ...
    useEffect(() => { if (!session || !joint) navigate('/'); }, [session, joint, navigate]);

    useEffect(() => {
        if (!joint) return;
        const initial: Record<string, number> = {};
        joint.movements.forEach(m => { initial[m.id] = 0; });
    ...
    const handleChange = useCallback((val: string | number) => {
        ...
        setMeasurements(p => ({ ...p, [activeId]: isNaN(numVal) ? 0 : numVal }));
    }, [activeId]);

    const handlePhoto = useCallback((angle: number) => {
    ...
    const currentJointIdx = queue.findIndex(q => q.jointId === jointId && q.side === side);
    ...
    const handleNextMovement = () => {
        if (currentMovIdx < totalMovSteps - 1) {
            ...
            const updated: RomSession = {
                ...session,
                measurements: { ...session.measurements, [jointId]: { ...session.measurements?.[jointId], [side]: measurements } },
            };
            saveRomSession(updated);
            setSession(updated);
            nextStep ? navigate(`/measure?joint=${nextStep.jointId}&side=${nextStep.side}`) : navigate('/results');
        }
    };
    ...
    const overallPct = (((currentJointIdx) + (currentMovIdx / totalMovSteps)) / totalJointSteps) * 100;
    ...
    return (
        <div className="rom-page">
            ...
            <header className="rom-header">
                <div>
                    <div className="rom-header__label">{side}측 {joint.name}</div>
                    <div className="rom-header__sub">{currentMovIdx + 1} / {totalMovSteps} 동작 측정 중</div>
                </div>
                <div className="rom-header__pct">
                    {Math.floor(overallPct)}<span style={{ fontSize: '0.75rem' }}>%</span>
                </div>
            </header>

            {/* Progress */}
            <div className="rom-progress">
                <div className="rom-progress__fill" style={{ width: `${overallPct}%` }} />
            </div>
            ...
            <footer className="rom-footer">
                <div className="rom-footer__inner">
                    <button onClick={handlePrevMovement} className="rom-footer__prev">이전</button>
                    <button onClick={handleNextMovement} className="rom-footer__next">
                        {currentMovIdx < totalMovSteps - 1 ? (
                            <span>다음 측정 이동</span>
                        ) : nextStep ? (
                            <span>
                                <span style={{ opacity: 0.5, marginRight: 4 }}>{nextStep.side}</span>
