...
const STAGE_CODE_MAP: Record<string, string> = {
    inhibit: 'H', lengthen: 'L', activate: 'A', integrate: 'I'
};
...
export const CesExercisePlayer: React.FC<CesExercisePlayerProps> = ({
    exercises, stageId, activeIndex, onIndexChange
}) => {
    ...
    if (exercises.length === 0) {
        return (
            <div className="flex items-center justify-center font-bold" style={{ height: '400px', color: 'var(--text-muted)' }}>
        ...
        <div className="flex flex-col h-full">
            <div className="main-header">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginTop: '0.35rem', flexWrap: 'wrap' }}>
                    {current.tools && (
                        <p className="main-subtitle">🛠 {current.tools}</p>
                    )}
                    {/* 세트/횟수/초 뱃지 */}
                    <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                        {current.sets && (
                            <span style={{ fontSize: '0.78rem', fontWeight: 700, padding: '0.2rem 0.55rem', borderRadius: '999px', background: 'rgba(28,63,111,0.1)', color: '#1C3F6F' }}>
                                {current.sets}세트
                            </span>
                        )}
                        {current.reps && (
                        ...
                        {current.holdSeconds && (
                            <div className="flex flex-col gap-2">
                                <span style={{ fontSize: '0.78rem', fontWeight: 700, padding: '0.2rem 0.55rem', borderRadius: '999px', background: 'rgba(99,230,190,0.15)', color: '#099268' }}>
                ...
                {current.description && (
                    <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginTop: '0.5rem', lineHeight: 1.6 }}>{current.description}</p>
                )}
            </div>

            <div className="overflow-hidden" style={{ borderRadius: '4px', background: '#000', boxShadow: '0 10px 30px rgba(0,0,0,0.1)' }}>
                <YoutubePlayer youtubeId={current.youtubeId} />
            </div>

            <div className="ex-tray">
                {exercises.map((ex, idx) => (
                    <div key={ex.id} className="ex-tray-item" onClick={() => onIndexChange(idx)}>
                        <div className={`ex-tray-thumb ${idx === activeIndex ? 'is-active' : ''}`}>
                            <img
